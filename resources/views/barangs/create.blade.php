...
<div class="container mx-auto px-4 py-10 max-w-3xl">
    ...
    <form action="{{ url('/barangs/store') }}" method="POST" class="space-y-4 bg-gray-800 p-6 rounded shadow">
        ...
        <div>
            ...
            <input type="text" name="nama_barang" required class="w-full bg-gray-700 text-white px-4 py-2 rounded">
        </div>
        ...
        <div>
            ...
            @foreach ($suppliers as $index => $supplier)
                <div class="mb-3">
                    <input type="checkbox" name="supplier_id[]" value="{{ $supplier->id }}" id="supplier_{{ $index }}" class="mr-2">
                    <label for="supplier_{{ $index }}">{{ $supplier->nama_supplier }}</label><br>
                    <input type="number" name="harga_beli[]" step="0.01" placeholder="Harga Beli" class="w-full bg-gray-700 text-white px-4 py-2 mt-1 rounded">
                </div>
            @endforeach
        </div>
